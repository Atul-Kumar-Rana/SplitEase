...
export default function EventCreate() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setUsers(data || []);
        // default: include self, but can be toggled off
        if (user?.id) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ✅ creator may or may not be included
    // but must have at least one other participant
    const othersCount = selectedUserIds.filter((id) => id !== user.id).length;
    if (othersCount < 1) {
      toast.error('Please add at least one other participant');
      return;
    }
